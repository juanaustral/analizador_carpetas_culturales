...
const CHANGELOG_DATA = [
  {
    ...
    date: "2026-07-04",
    entries: [
      { scope: "Nuevas convocatorias FNA", items: [
        "Agregados 2 concursos: Composicion ANBA-FNA 2026 y Valoracion Patrimonial 2026",
        "Selector de concurso FNA dentro de la card del organismo",
        "Criterios de evaluacion extraidos del reglamento oficial ANBA-FNA"
      ]},
      { scope: "Nuevas convocatorias Ibermusicas", items: [
        "Agregadas 15 lineas de convocatoria Ibermusicas 2026",
        "Selector de linea Ibermusicas dentro de la card del Ministerio de Cultura",
        "Criterios extraidos de las bases oficiales de ibermusicas.org"
      ]},
      { scope: "Backend", items: [
        "Server actualizado: acepta fnaLine y minculturaLine en el request",
        "System prompt ampliado con criterios detallados para FNA e Ibermusicas"
      ]},
      { scope: "Changelog modal", items: [
        "Version badge ahora es cliqueable y muestra historial de cambios",
        "Permite verificar la version actual del sitio"
      ]},
    ]
  },
  {
    ...
    date: "2026-06-12",
    entries: [
      { scope: "SEO y Meta", items: ["Meta tags, Open Graph, JSON-LD, favicon, robots.txt, sitemap.xml"] },
      { scope: "Correcciones", items: ["Contador de visitas migrado a countapi.mileshilliard.com"] },
    ]
  },
  {
    version: "1.2",
    date: "2026-06-12",
    entries: [
      { scope: "Colores en resultados", items: ["Puntos fuertes en verde, debiles en rojo, sugerencias en amarillo"] },
      { scope: "Botones y exportacion", items: ["Descargar PDF, TXT, Copiar informe, Compartir con attribution"] },
      { scope: "Interfaz", items: ["Cards clickeables, version badge refinado, disclaimer actualizado"] },
      { scope: "Limite de API", items: ["Manejo de cuota agotada y errores 429"] },
    ]
  },
  {
    version: "1.1",
    date: "2026-06-12",
    entries: [
      { scope: "Nuevas funcionalidades", items: ["Medidor de API, feedback con captcha, keep-alive, contador de visitas"] },
      { scope: "Infraestructura", items: ["Migracion a GitHub juanaustral, deploy en Render Free"] },
    ]
  },
  {
    version: "1.0",
        ...
        <div id="header-inner" className="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex flex-col items-end gap-1 pt-1 sm:pt-0">
            <button
              ...
              className="px-3 py-1 bg-neutral-100 border border-neutral-300 text-neutral-600 rounded-none text-[9px] font-mono font-bold tracking-wider cursor-pointer hover:bg-neutral-200 hover:border-neutral-900 hover:text-neutral-950 transition-colors"
        ...
        {showChangelog && (
          ...
          >
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.98, y: 10 }}
              transition={{ duration: 0.2 }}
              className="bg-white w-full max-w-xl rounded-none border border-neutral-900 flex flex-col shadow-lg max-h-[80vh]"
            >
              <div className="bg-neutral-950 border-b border-neutral-900 p-4 text-[#dae122] flex justify-between items-center shrink-0">
                <div className="flex items-center gap-2.5">
                  <span className="w-2 h-2 bg-[#dae122] shrink-0"></span>
                  <h3 className="font-mono font-bold text-xs uppercase tracking-widest text-[#dae122]">
                    HISTORIAL DE CAMBIOS — VERSION {VERSION}
                  </h3>
                </div>
                <button 
                  type="button" 
              ...
              <div className="p-6 space-y-5 flex-1 overflow-y-auto select-text">
                {CHANGELOG_DATA.map((release) => (
                  <div key={release.version} className={`space-y-2 ${release.version !== CHANGELOG_DATA[0].version ? 'pt-4 border-t border-neutral-200' : ''}`}>
                    <div className="flex items-center gap-2.5">
                      <span className={`px-2 py-0.5 text-[9px] font-mono font-bold tracking-wider ${
                        release.version === CHANGELOG_DATA[0].version
                          ? 'bg-[#dae122] text-neutral-950 border border-neutral-900'
                      ...
                      {release.version === CHANGELOG_DATA[0].version && (
                        <span className="text-[8px] font-mono font-bold text-green-700 bg-green-100 border border-green-300 px-1.5 py-0.5">
                          ACTUAL
                        </span>
                      )}
                    </div>
                    <div className="space-y-2 pl-1">
                      {release.entries.map((entry, ei) => (
                        <div key={ei} className="space-y-0.5">
                          <h4 className="text-[10px] font-mono font-bold text-neutral-950 uppercase tracking-wider">
                            {entry.scope}
                          </h4>
                          <ul className="space-y-0.5">
                            {entry.items.map((item, ii) => (
                              <li key={ii} className="text-[10px] text-neutral-600 font-medium leading-relaxed flex items-start gap-1.5 pl-3">
                                <span className="w-1 h-1 bg-neutral-400 shrink-0 mt-1.5"></span>
                                {item}
                              </li>
                            ))}
                          </ul>
              ...
              <div className="bg-neutral-50 p-4 border-t border-neutral-200 flex justify-between items-center shrink-0">
                <span className="text-[9px] font-mono text-neutral-400">
                  {CHANGELOG_DATA.length} versiones registradas
                </span>
                <button
                  type="button"
                  onClick={() => setShowChangelog(false)}
